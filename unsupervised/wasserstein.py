...
class WassersteinAutoEncoder(nn.Module):
    ...
    def __init__(self, ksi=10., latent_dimension=2, activation=nn.ReLU(),
        ...
        """
        Implementation of the Wasserstein auto-encoder, using the MMD loss.

        For a detailed presentation of the algorithm, read the original paper by Tolstikhin et al (https://arxiv.org/abs/1711.01558).
    ...
    def encode(self, x):
        """
        Encoder in the WAE architecture. Takes a 32x32 image as input and returns its latent representation.
    ...
    def loss(self, x, x_tilde, z):
        ...
        z_fake = torch.randn(n, self.latent_dimension).to(device)
    ...
    def kernel(self, x, y):
        ...
        Args:
            ...
            y (torch.Tensor): a PyTorch Tensor

        Returns:
            ker (torch.Tensor): The kernel computed for every pair of x and y.
        """
